...
cd <- xlsx::read.xlsx(pt, sheetIndex = 1) %>% as_tibble() %>% mutate_all(as.character)
...
cd <- cd[4:nrow(cd),] %>% mutate_at(2:7, as.numeric) %>% mutate_at(c(2,5), function(x) x*100)
r <- which(cd[,1] == "киев 18+")

all <- cd %>% slice(1:(r-1))
kyiv <- cd %>% slice((r+1):nrow(cd))
